...
void main() {
  ...
  group("deserializeS16", () {
    test("throws the expected exception when the iterator ends immediately",
        () {
      expect(
          () => deserializeS16(<U8>[].iterator, "Example Description"),
          throwsA(predicate((e) =>
              e is StateError &&
    ...
    test("throws the expected exception when the iterator ends after one U8",
        () {
      expect(
          () => deserializeS16(<U8>[0xce].iterator, "Example Description"),
          throwsA(predicate((e) =>
              e is StateError &&
              e.message == "Example Description - Unexpected end of file.")));
    });
    ...
    test("throws the expected exception when the iterator includes invalid U8s",
        () {
      expect(
          () => deserializeS16(<U8>[0xce, 300].iterator, "Example Description"),
          throwsA(predicate((e) =>
              e is RangeError &&
              e.message ==
    ...
    test("returns the expected S16 when the iterable ends", () {
      expect(deserializeS16([0xce, 0x80].iterator, "Example Description"),
          equals(-32562));
    });
    ...
    group("when the iterable does not end", () {
      ...
      setUpAll(() {
        iterator = [0xce, 0x80, 0x48, 0x07, 0xe5, 0x6e].iterator;
        output = deserializeS16(iterator, "Example Description");
      });
